...
{{
  ...
  "cultural_context_guidelines": "这本书中可能出现哪些中国读者不熟悉的文化背景知识 (如政治制度、社会习俗、历史事件、地名文化含义等), 列出 2-4 条具体类型"
}}
...
def _default_strategy() -> dict:
    return {
        ...
        "quantity_hint": 5,
        "cultural_context_guidelines": "政治制度、社会习俗、历史事件、地名文化含义等中国读者可能不熟悉的背景知识",
    }
...
3b. **文化背景词**: 额外识别文中**作者未解释、但对中国读者不熟悉**的文化背景词 ({cultural_guidelines})。这类词 category 标为 "cultural_context"。definition_para 设为该词首次出现的段落编号。
...
- **文化背景词 (category=cultural_context)**: 用简明中文为中国读者解释该文化背景, 包括它是什么、在其所属文化中意味什么. 不要求使用作者原话.
